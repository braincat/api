...
import com.structurizr.onpremises.workspace.WorkspaceComponentException;
import com.structurizr.onpremises.workspace.WorkspaceSummary;
...
public class ApiServletTests {
    ...
    public void test_doPost_ReturnsAnApiError_WhenTheWorkspaceCouldNotBeCreated() throws Exception {
        ...
        apiServlet.doPost(request, response);
        assertEquals(500, response.getStatus());
        assertEquals("{\"message\":\"A key and secret pair for workspace 1234 already exists.\"}", response.getContent());
    }
    ...
    public void test_doPost_ReturnsAnApiSuccessMessage_WhenTheWorkspaceCouldBeCreated() throws Exception {
        ...
        assertEquals("{\"message\":\"The key and secret for workspace 1234 have been updated.\"}", response.getContent());
    }
...
class MockWorkspaceComponent implements WorkspaceComponent {
    ...
    public RenderedImage getImage(long workspaceId, String name) throws WorkspaceComponentException {
        try {
            return ImageIO.read(new File("test/unit/com/structurizr/onpremises", name));
        } catch (IOException e) {
